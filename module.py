...
def instance_norm_layer(inputs, epsilon = 1e-06, activation_fn = None, name = None):
    instance_norm_layer = tf.contrib.layers.instance_norm(inputs = inputs, epsilon = epsilon,\
...
def conv1d_layer(inputs, filters, kernel_size, strides = 1, padding = 'same', activation = None,\
    ...
    conv_layer = tf.layers.conv1d(inputs = inputs, filters = filters, kernel_size = kernel_size,\
